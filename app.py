...
def load_from_database(option_type):
    ...
    try:
        ...
        if option_type == "covered_call":
            ...
            data_df['moneyness'] = (data_df['price'] - data_df['strike']) / data_df['price'] * 100
            data_df['net_profit'] = (data_df['bid'] * 100) - ((100 * data_df['price']) - (100 * data_df['strike']))
...
if scan_button:
    with st.spinner("Running scan..."):
        ...
        if results is None or results.empty:
            st.warning("No data available in database. Please click 'Refresh Data from Yahoo Finance' in the sidebar.")
            st.session_state.scan_results = None
        else:
            # Apply price filters
            results = results[
